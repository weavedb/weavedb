...
wait_and_ping_service() {
    ...
    ps aux | grep -E "beam.smp.*10001" | grep -v grep
    return 1
}
...
while true; do
    ...
    while kill -0 $CHILD_PID 2>/dev/null; do
        # Get memory usage of all beam.smp processes
        BEAM_PIDS=$(pgrep -f "beam.smp.*10001" | head -1)
        ...
        if [ -n "$BEAM_PIDS" ]; then
            ...
            if [ -n "$MEM_KB" ] && [ "$MEM_KB" -gt 0 ]; then
                MEM_MB=$((MEM_KB / 1024))
                echo "$(date): Memory: ${MEM_MB}MB / ${MAX_MEM_MB}MB (PID: $BEAM_PIDS)"
            ...
            # Fallback to monitoring the shell process
            MEM_KB=$(ps -o rss= -p $CHILD_PID 2>/dev/null | tr -d ' ')
            if [ -n "$MEM_KB" ] && [ "$MEM_KB" -gt 0 ]; then
                MEM_MB=$((MEM_KB / 1024))
                echo "$(date): Shell Memory: ${MEM_MB}MB / ${MAX_MEM_MB}MB"
            fi
        fi
