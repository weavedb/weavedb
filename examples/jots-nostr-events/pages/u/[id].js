  ...
  useEffect(() => {
    if (!isNil(router.query.id)) {
      ;(async () => {
        ...
        let user = null
        if (!isNil(ev)) {
          let _user = JSON.parse(ev.content)
          user = {
            cover: _user.banner,
            name: _user.display_name === "" ? _user.name : _user.display_name,
            handle: _user.name,
            image: _user.picture,
            description: _user.about,
            address: ev.pubkey,
          }
        }
        /*
    ...
    <ChakraProvider>
      ...
      <EditUser
        {...{
          ...
          setUser,
          user,
          setPuser: puser?.address === user?.address ? setPuser : null,
        }}
      />
